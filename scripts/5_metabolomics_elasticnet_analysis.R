...
pacman::p_load(readr, dplyr, janitor, stringr, naniar, broom, 
               ...
               caret, MLmetrics, foreign, parallel, doParallel, readr)

# # Import data -----
# t1 = Sys.time()
# metabolomics = read.csv("/mnt/project/data/raw/Biomarkers.csv")
# 
...
# t2 = Sys.time()
# t2-t1
# # Time difference of 2.361004 mins
# rm(t1, t2)
# 
# names = readr::read_tsv("/mnt/project/data/raw/fieldsum.tsv", show_col_types = FALSE)
# 
# # Preprocessing -----
# ## Change the format of column names ----
# metabolomics = metabolomics %>%
#   dplyr::rename_with(~ .x %>%
#                        stringr::str_replace_all("X", "n_") %>%
...
# 
# data = dplyr::left_join(gdm, metabolomics, by = "eid") %>%
...
#   dplyr::select(eid, ethnicity, #country,
#                 townsend_q5, education_group, diab_fam,
...
# # [1] 0
# 
# # saveRDS(data, "./upload/2025_07_16_metabolomics_dataset_elasticnet.RDS")

## Dummies ------

data = read_rds("./upload/2025_12_15_metabolomic_dataset_limma_elasticnet.RDS")
...
t1 = Sys.time()
set.seed(123456789)
elasticnet_under = train(x = data[, c(1:187)], y = data$gdm_final, 
                         method = "glmnet",   
                         family = "binomial",
                         penalty.factor = c(rep(0, 5), rep(1, 182)),
                         metric = "GMean",
                         maximize =  TRUE,
                         trControl = control, 
...
t2 = Sys.time()
t2 - t1
# Time difference of 3.693026 mins
results = elasticnet_under$results

## Save the model as R object ----
# saveRDS(elasticnet_under, "./upload/2_elasticnet/elasticnet_metabolomics.RDS")

# Final Predictive Model ------

elasticnet_under = readr::read_rds("./upload/elasticnet_metabolomics.RDS")
...
t2 = Sys.time()
t2 - t1
# Time difference of 2.406026 secs

## Save the model as R object ----
# saveRDS(elasticnet_under, "./upload/final_elasticnet_metabolomics.RDS")

elasticnet_under = read_rds("./upload/final_elasticnet_metabolomics.RDS")

pred_under = predict(elasticnet_under, data[, 1:187])
...
coef_elasticnet_under %>% 
  ...
  # labs(title = "Metabolomic signature associated with GDM")#, 
